...
contract DBTokenSell is SaleFactory {
    ...
    function finalizeOffer(string memory eventCode, string memory offerId) public {
        ...
        require(offer.offeringUser == _msgSender(), "DBTokenSell: only offering user can finalize the offer");
        require(offer.status == OfferStatus.Open, "DBTokenSell: offer is not open for bids");
    ...
    function bidOnOffer(
        ...
        require(offer.status == OfferStatus.Open, "DBTokenSell: offer is not open to bids");
        require(bidAmount >= offer.minStandardTokensRequested, "DBTokenSell: cannot offer less than minimum requested");
        require(bidAmount > getHighestBid(offer), "DBTokenSell: must bid more than the current highest bid");
        require(stdToken.balanceOf(_msgSender()) >= highestBidDiff, "DBTokenSell: insufficient token amount");
        require(
