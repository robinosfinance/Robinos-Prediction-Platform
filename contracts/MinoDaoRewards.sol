...
abstract contract RecordingRewards {
    struct Reward {
        uint256 prize;
        bool honorary;
        address[] winningMembers;
    }

    Reward[] private allRewards;

    function recordReward(
        uint256 prize,
        bool honorary,
        address[] memory winningMembers
    ) internal {
        allRewards.push(Reward(prize, honorary, winningMembers));
    }

    /**
     * @dev Returns an array of all successful rewards initialized
     * by the owner.
     *
     * NOTE: users which have multiple tokens that were eligible
     * for a reward will appear once for each eligible token in
     * the {winningMembers} array.
     *
     * @return {Reward[]} array of structs
     *  (uint256 prize, bool honorary, address[] winningMembers)
     */
...
abstract contract StoringERC20Instance is Ownable {
    ERC20 internal rewardToken;

    constructor(ERC20 rewardToken_) {
        rewardToken = rewardToken_;
    }

    /**
     * @dev Allows owner to update ERC20 contract instance used as the reward token
     */
    function setRewardTokenInstance(ERC20 rewardToken_) public onlyOwner {
        rewardToken = rewardToken_;
    }
}

contract MinoDaoRewards is StoringMinoDaoInstance, StoringERC20Instance, RecordingRewards {
    constructor(MinoDao minoDao_, ERC20 rewardToken_)
        StoringMinoDaoInstance(minoDao_)
        StoringERC20Instance(rewardToken_)
        Ownable()
    {}

    function distributeRewardToTokenOwners(uint256 reward, uint256[] memory tokenIds)
        private
        returns (address[] memory)
    {
        require(rewardToken.balanceOf(_msgSender()) >= reward, "MinoDaoRewards: insufficient balance for reward");
        require(
            rewardToken.allowance(_msgSender(), address(this)) >= reward,
            "MinoDaoRewards: insufficient allowance for reward"
        );

        rewardToken.transferFrom(_msgSender(), address(this), reward);

        uint256 rewardPerMember = reward / tokenIds.length;
        uint256 rewardLeftOver = reward - (rewardPerMember * tokenIds.length);
        address[] memory winners = new address[](tokenIds.length);

        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 adjustedReward = rewardPerMember;
            address tokenOwner = minoDao.ownerOf(tokenIds[i]);

            if (rewardLeftOver > 0) {
                // Since Solidity can only work with integers
                // We have to increment some of the individual rewards,
                // so we deplete the whole reward pool
                adjustedReward++;
                rewardLeftOver--;
            }

            rewardToken.transfer(tokenOwner, adjustedReward);
            winners[i] = tokenOwner;
        }
    ...
    /**
     * @dev Allows owner to distribute a reward to all users holding a MinoDao token.
     * Users which hold multiple tokens will receive a reward for each one of the held tokens.
     *
     * @param totalReward for the entire group of members, which is split evenly for each user
     */
    function distributeReward(uint256 totalReward) public onlyOwner {
        address[] memory winners = distributeRewardToTokenOwners(totalReward, minoDao.getTokenIds());
        recordReward(totalReward, false, winners);
    }

    /**
     * @dev Allows owner to distribute a reward to all users holding an honorary MinoDao token.
     * Users which hold multiple honorary tokens will receive a reward for each one of the held tokens.
     *
     * @param totalReward for the entire group of members, which is split evenly for each honorary user
     */
    function distributeHonoraryReward(uint256 totalReward) public onlyOwner {
        address[] memory winners = distributeRewardToTokenOwners(totalReward, minoDao.getHonoraryTokenIds());
        recordReward(totalReward, true, winners);
    }
}
