...
/***********************************************************************
 ***********************************************************************
 ***********        ROBINOS GOVERNANCE TOKEN NFT STAKE       ***********
 ***********************************************************************
 **********************************************************************/
    ...
    HandlingTime
{
    ...
    constructor(
        ...
        minUserStakeDuration = toUnixTime(period_, stakeDuration_);
    }
    ...
    function setRewardToken(IERC20 rewardToken_) public onlyOwner noActiveSale {
        rewardToken = rewardToken_;
    }
    ...
    function setBaseReward(string memory eventCode, uint256 amount) private {
        ...
        private
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            stakingToken.safeTransferFrom(address(this), user, tokenIds[i]);
        }
    }

    function recordStakedToken(
        string memory eventCode,
        address user,
        uint256 tokenId
    ) private {
        UserStaked storage userStake = getUserStakeData(eventCode, user);
        if (userStake.stakedAt == 0) userStake.stakedAt = time();
    ...
    function removeEventStakedTokens(
        ...
        for (uint256 i = 0; i < length; i++) {
            if (
                ...
                eventStakedTokens[eventHash][i - deleted] = copyTokenIds[i];
            }
        }
    }
        ...
        returns (uint256)
    {
        ...
        for (uint256 daysPassed = 2; daysPassed <= 5; daysPassed++) {
            if (timePassed <= toUnixTime("days", daysPassed)) return multiplier;
            multiplier *= 2;
        }
        return multiplier;
    }
