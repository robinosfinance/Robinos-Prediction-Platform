...
contract SideBetV6 is SaleFactory {
    ...
    // Event to log new unique interactions
    ...
    function deposit(
        ...
        if (!hasInteracted[sender]) {
            // Mark the sender as having interacted
            hasInteracted[sender] = true;
            // Add the sender to the array of unique wallets
            uniqueWallets.push(sender);
            // Emit an event for the new unique interaction
    ...
    function getUniqueWalletCount() public view returns (uint256) {
        return uniqueWallets.length;
    }

    // Function to return all unique wallet addresses that interacted
    function getAllUniqueWallets() public view returns (address[] memory) {
        return uniqueWallets;
    }

    // Function to return the total token spent for a specific token
    function getTotalTokenSpent(address tokenAddress) public view returns (uint256) {
