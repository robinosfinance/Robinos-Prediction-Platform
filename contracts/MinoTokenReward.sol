...
contract MinoTokenReward is Ownable {
    MinoToken private minoToken;
    ...
    mapping(address => bytes32[]) private userRewardHashes;
    mapping(bytes32 => address[]) private rewardWinners;
    bytes32[] private allRewardHashes;

    constructor(MinoToken minoToken_) Ownable() {
        minoToken = minoToken_;
    }

    /**
     * @dev Creates a new reward. Will immediately check if this contract has enough balance
     * on the given rewardToken contract and send the rewardAmount of funds towards each
     * user who is an owner of a mino token described with playerName and sportsName.
     * The reward is later stored under a unique hash using rewardName and playerName.
     * @param rewardName the name of the reward. This and player name must be a unique pair
     * @param playerName name of the winning token
     * @param sportsName uniquely pair with playerName to select rewardable tokens
     * @param rewardToken ERC20 contract instance to use as reward for users
     * @param rewardAmount amount per user of rewardToken tokens to be sent as reward
     */
    function newReward(
        ...
        bytes32 rewardHash = StringHash.dualHash(rewardName, playerName);
        require(rewards[rewardHash].rewardAmount == 0, "MinoTokenRewards: reward already created");
        require(rewardAmount != 0, "MinoTokenRewards: reward is zero");

        MintedToken[] memory tokens = minoToken.getAllTokensByNameAndSport(playerName, sportsName);

        require(
            rewardAmount * tokens.length <= rewardToken.balanceOf(address(this)),
            "MinoTokenRewards: insufficient reward token balance"
        );

        for (uint256 i = 0; i < tokens.length; i++) {
            address tokenOwner = minoToken.ownerOf(tokens[i].tokenId);
            rewardToken.transfer(tokenOwner, rewardAmount);
            userRewardHashes[tokenOwner].push(rewardHash);
            rewardWinners[rewardHash].push(tokenOwner);
        }

        allRewardHashes.push(rewardHash);
        rewards[rewardHash] = Reward(rewardName, playerName, sportsName, rewardToken, rewardAmount);
    }

    /**
     * @dev Returns all initial data of all rewards initialized on this contract
     */
    function getAllRewards() public view returns (Reward[] memory) {
        bytes32[] memory rewardHashes = allRewardHashes;
        Reward[] memory allRewards = new Reward[](rewardHashes.length);

        for (uint256 i = 0; i < rewardHashes.length; i++) {
            allRewards[i] = rewards[rewardHashes[i]];
        }
    ...
    /**
     * @dev Returns all rewards won by user
     */
    function getUserRewards(address user) public view returns (Reward[] memory) {
        bytes32[] memory rewardHashes = userRewardHashes[user];
        Reward[] memory userRewards = new Reward[](rewardHashes.length);

        for (uint256 i = 0; i < rewardHashes.length; i++) {
            userRewards[i] = rewards[rewardHashes[i]];
        }
     ...
     */
    function getRewardWinners(string memory rewardName, string memory playerName)
        public
        view
        returns (address[] memory)
    {
        bytes32 rewardHash = StringHash.dualHash(rewardName, playerName);
        require(rewards[rewardHash].rewardAmount != 0, "MinoTokenRewards: reward not found");

        return rewardWinners[rewardHash];
    }
}
